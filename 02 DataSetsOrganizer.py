...
# -*- coding: utf-8 -*-
"""
RGB YOLO Dataset Builder (GUI) — Extended Version
- Only RGB (.jpg/.jpeg/.png + .txt with the same name)
- Extended .txt validation (bbox in [0,1] AND x±w/2, y±h/2 within the image boundaries)
- Optional control over the number of classes (0..N-1)
- Class counts (global histogram) + CSV export
- Split per subfolder into train/valid/test (as before)
- NEW: "Knockout folder" — fine-tune global aspect ratios
- Optional trimming of the number of series used to achieve the ideal ratio
- Transfer (cut) to RGB/{images,labels}/{train,valid,test}
- CSV reports (split + classes)
"""
...
APP_TITLE = "RGB YOLO Dataset Builder"
RANDOM_SEED = 42
